...
final class SessionTests: XCTestCase {
  ...
  func testFormattedDate() {
    let fixedDate = Date(timeIntervalSince1970: 1_704_067_200)  // 2024-01-01 00:00:00 UTC
    let session = Session(
      id: "test-id",
      sessionId: "session-123",
      displayName: "Test",
  ...
  func testFormattedRelativeDateJustNow() {
    ...
    XCTAssertTrue(
      session.formattedRelativeDate == "just now" || session.formattedRelativeDate.contains("m ago")
    )
  }
